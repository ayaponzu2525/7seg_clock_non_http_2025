...
void ShowTime(int hour, int minute) {
  ...
  for (int i = 0; i < 4; i++) { // 各桁
    for (int j = 0; j < digit; j++) { // 1桁分のLED
      int index = i * digit + j;
      int hue = digit_hues[i];    // 桁ごとの色を使う
      int sat = 255;
      int val = 200;
  ...
  for (int i = 0; i < coron_number; i++) {
    pixels.setPixelColor(coron_digits[i], pixels.Color(flag*100, flag*100, flag*100));
  }
